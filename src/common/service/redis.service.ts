...
class RedisService {
  ...
  //* The SocketKey Method Function To generate a Redis key for storing socket IDs based on the user's ID
  socketKey(userId: Types.ObjectId) {
    return `user:Socket:${userId}`;
  }

  //* Method to add a socket ID to the Redis set for a specific user
  async addSocket({
    userId,
    SocketId,
  }: {
  ...
  //* Method to remove a socket ID from the Redis set for a specific user
  async removeSocket({
    userId,
    SocketId,
  }: {
  ...
  //* Method to get all socket IDs from the Redis set for a specific user
  async getSockets(userId: Types.ObjectId) {
    return await this.client.sMembers(this.socketKey(userId));
  }

  //* Method to check if there are any socket IDs in the Redis set for a specific user
  async hasSockets(userId: Types.ObjectId) {
    return await this.client.sCard(this.socketKey(userId));
  }

  //* Method to remove all socket IDs from the Redis set for a specific user, effectively clearing the set
